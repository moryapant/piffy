...
class SimpleVisitMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            ...
            // Super simple - just insert directly into the database
            try {
                $result = \DB::table('visits')->insert([
                    ...
                    'activity_type' => 'page_view', // Setting the activity type for page views
                    'visited_at' => now(),
                    'created_at' => now(),
                    'updated_at' => now()
                ]);
                
                file_put_contents($logPath, date('[Y-m-d H:i:s] ') . "Database insert result: " . ($result ? 'Success' : 'Failed') . PHP_EOL, FILE_APPEND);
            } catch (\Exception $e) {
                file_put_contents($logPath, date('[Y-m-d H:i:s] ') . "Error inserting visit: " . $e->getMessage() . PHP_EOL, FILE_APPEND);
            }
